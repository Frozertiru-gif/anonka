...
export interface CreateBridgeOptions {
  /** Interactive Telegram prompts are permitted only for explicit login setup. */
  interactive?: boolean;
  /** Isolates a creator's Telegram session from all other creators. */
  creatorId?: string;
}

export function createBridge(config: Config, options: CreateBridgeOptions = {}): ITelegramBridge {
  ...
  return new GramJSUserBridge({
    ...
    sessionPath: resolveCreatorSessionPath(config.telegram, options.creatorId),
